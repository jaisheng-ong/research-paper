...
The sole prediction target of this study is the Chemical Oxygen Demand within the aeration tank (\texttt{AT\_COD}), which quantifies the residual organic load at the biological treatment stage and is the primary operational indicator of treatment performance. Although Dissolved Oxygen (\texttt{AT\_DO}) is listed in the raw dataset and is physically relevant to the aeration process, it was excluded from the modelling pipeline during preprocessing and is not a prediction target in this study. Table~\ref{tab:variables} summarises the full set of measured variables, their abbreviations, and their roles.
...
Prior to modelling, a structured preprocessing pipeline was applied to reduce the 25 available sensor variables to a clean predictor set. First, \texttt{AT\_DO} was excluded based on domain considerations, and the raw microbial count columns (\texttt{c1}--\texttt{c8}) were removed due to measurement unreliability, reducing the candidate feature set to 16 variables. Second, Variance Inflation Factor (VIF) analysis was conducted to detect multicollinearity. The variable \texttt{ET\_COD} was identified with a VIF score of 21.14, far exceeding the standard threshold of 10, and was subsequently dropped. This yielded a final set of \textbf{15 core predictor variables} fed into all downstream models.
